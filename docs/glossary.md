...
| 点灯色 | 発光色、表示色 | URLの `c`。作成画面のラベルは「画面の点灯色」 |
...
**説明**: 作成画面に「メッセージの長さ」として、秒単位で切り上げた整数(URLの `p` と同じ値)で表示する唯一の時間(モールス信号を知らない人にも分かるよう、画面では「1周」という言葉を使わない)。60秒を超えるとQRコードを生成しない
...
**説明**: 1周分の所要時間を秒単位で切り上げた値(1〜60秒の整数)を内部で自動決定する。60の約数でなくてもよい(2026-09-25 までの版は60の約数に限っていた)。ユーザーは入力も確認もしない(画面に表示しない)。URLの `p` に入り、参加側はその値をそのまま使う

**関連用語**: 開始時刻、1周分の所要時間、周期の自動決定

**使用例**:
- 所要時間8.5秒 → 周期9秒(区切りの後、0.5秒は消灯のまま待つ)
- 所要時間31.2秒 → 周期32秒
...
**説明**: 共通の起点(UNIX時刻0)から周期の倍数の時刻なので、周期が何秒でも、同じ時計を持つ全端末で一致する。参加した直後は、次に来る開始時刻まで待機中となる(最大で周期1回分)

**使用例**:
- 周期15秒なら、毎分0秒・15秒・30秒・45秒が開始時刻
- 周期32秒なら毎分0秒とは限らない(例: 10時00分16秒・48秒、10時01分20秒…)が、全端末で一致する
...
**本プロジェクトでの使用**: 開始時刻の計算の基準。全端末が同じ起点から数えるので、周期で割り切れる時刻はどの端末でも一致する
...
**制約**: `periodSec × 1000 ≥ 1周分の所要時間`、`periodSec` は1〜60の整数
...
| `INVALID_PERIOD` | `p` が1〜60の整数でない、または所要時間より短い | QRコードの内容が正しくありません |
...
**定義**: 1周分の所要時間を秒単位で切り上げる(最小1秒、60秒を超えたら生成しない)

**計算式**:
```
周期(秒) = max(1, ceil(1周分の所要時間(ms) ÷ 1000))
1周分の所要時間が60,000msを超える → 60秒に収まらない(QRコードを生成しない)
```
