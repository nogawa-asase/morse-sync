...
- **検証内容**: 機能設計書「アルゴリズム5」の通り、バージョン・文字種・メッセージ(文字数と対応文字)・1拍(整数と範囲)・色(16進6桁)・周期(60の約数と所要時間)
...
### 統合テスト
- **方法**: ローカルの静的サーバー(`npx serve public` 等、開発時のみ)で開き、ブラウザで手動確認する
...
- トーチの切替には端末ごとの遅延がある(P1。1拍300ms以上を推奨)
