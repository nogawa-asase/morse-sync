...
interface CreateInput {
  ...
  color: string;     // '#ffcc00' 形式(カラーピッカーの値)
}
```
...
stateDiagram-v2
    ...
    参加画面 --> 作成画面: 「パターンを作る」(ハッシュを消し、設定を引き継ぐ)
    読込エラー --> 作成画面: 「パターンを作る」
    参加画面 --> 判定: hashchange
```
...
| 1周の長さ | 末尾の区切りを含む1周分の所要時間 | `14.0秒`(小数第1位、四捨五入) |
...
| 共有・コピー失敗 | 両画面 | URLを選択可能なテキストで表示 | 共有できませんでした。URLをコピーしてください |
